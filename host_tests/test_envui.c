 ...
 * is big enough to read across a room -- and renders of every page in every
 * state to look at beside docs/design/envo-ui/ref/.
 *
...
static void test_reading_pages(void)
{
    ...
    expect("the corners stay dark", corners_dark(fb));
    /* The block begins its pad left of the margin so the letters on it line
       up with the number's at x 16. */
    expect("POOR's block starts left of the text margin, its letters on it",
           count(fb, 12, 96, 15, 132, POOR) > 100 && count(fb, 0, 90, 11, 136, POOR) == 0);
    ...
    /* "37" alone could be a temperature from across the room: the percent
       sign stands after it, top-aligned, half its size -- white ink in the
       upper half of the number's band, right of the two digits. */
    {
        int top, bottom;
        ink_rows(fb, 100, 20, 150, 90, &top, &bottom);
        printf("     the percent sign's ink: rows %d..%d\n", top, bottom);
        expect("humidity's number carries a percent sign", top >= 24 && top <= 30 && bottom <= 62);
    }
...
static void test_detail_pages(void)
{
    ...
    expect("the POOR line runs unbroken across the plot",
           count(fb, 16, 68, 246, 68, POOR) == 231);
    /* The plot stops 10 px short of the key, so the now-dot is not a bullet
       on the key word level with it. */
    expect("a clear gap between the now-dot and the key", dark(fb, 251, 44, 255, 141));
    ...
    {
        int top, bottom;
        expect("no white trace above the FAIR line", ink_rows(fb, 16, 44, 246, 97, &top, &bottom) == 0);
    }
    expect("no amber or red trace below it",
           count(fb, 16, 102, 246, 141, FAIR) == 0 && count(fb, 16, 102, 246, 141, POOR) == 0);
     ...
     * line, and a grey stem ties it back down (column x 180, slot 205).
     */
    make_day(0);
    ...
    expect("a floating peak is stemmed to its trace", count(fb, 180, 52, 180, 92, GREY) >= 30);
    expect("and the stem passes behind the zone lines",
           count(fb, 16, 68, 246, 68, POOR) == 231 && count(fb, 16, 100, 246, 100, FAIR) >= 200);
    make_day(2500);
...
static void test_edge_series(void)
{
    ...
    expect("an empty chart has no GOOD tint", count(fb, 0, 0, W - 1, H - 1, TINT) == 0);
    {
        ...
        expect("the zone lines run whole behind it",
               count(fb, 16, 68, 246, 68, POOR) == 231 && count(fb, 16, 100, 246, 100, FAIR) == 231);
