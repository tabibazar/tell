...
| LilyGO T-Display-S3 | ST7789 320×170, 8-bit i80 | 26 × 7 | Verified |
| Waveshare ESP32-S3-LCD-1.47B | ST7789 320×172, SPI | 26 × 7 | Verified |

The **Waveshare 1.47B** has a QMI8658 soldered on, so it runs the tilt-driven
`Sand` page out of the box — see
[docs/hardware/waveshare-lcd-147b.md](docs/hardware/waveshare-lcd-147b.md).
The Feather runs it too, from a breakout on its STEMMA QT port. The
**T-Display-S3** has no sensor at all and shows the clock and messages only:
...
CrowPanel, and **26 by 7** on the T-Display-S3 and the Waveshare 1.47B. Text wraps at word boundaries; a
