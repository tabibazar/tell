#!/bin/sh
# Flash visio (Waveshare ESP32-S3-Touch-LCD-3.5B) with the app built in build-visio/.
#
#   tools/flash-visio.sh                  app only, finds the usbmodem port
#   tools/flash-visio.sh --full           bootloader + partition table + app
#   tools/flash-visio.sh /dev/cu.usbmodem1101
#
...
# Which board is actually on that port. visio and wave both appear as
# /dev/cu.usbmodem*, so the port is no evidence, and these firmwares are not
# interchangeable.
. tools/board-guard.sh
board_guard visio "$PORT" "$ANY"

if [ -n "$FULL" ]; then
    ARGS="0x0 build-visio/bootloader/bootloader.bin
          0x8000 build-visio/partition_table/partition-table.bin
          0x10000 $BIN"
else
    ARGS="0x10000 $BIN"
fi
...
sleep 6
DEVICE="${DEVICE:-visio}"
# visio has a PCF85063 RTC on board, but it is not wired into the firmware yet,
