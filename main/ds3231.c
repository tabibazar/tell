...
/*
 * The chip itself, on every board but watch and speaker. Those two carry a
 * PCF85063 instead, and pcf85063.c answers to these same names there so
 * neither main.c nor speaker_app.c changes; only the pure helpers above are
 * shared. The choice is made here and in pcf85063.c as well as in CMake, so
 * the two files can never both define the chip's names.
 */
#ifdef ESP_PLATFORM
#include "sdkconfig.h"
#endif
